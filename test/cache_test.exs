defmodule CacheTest do
  ...
  test "put(cache) full cache=> removes the lru_entry", ctx do
    cache = ctx[:c]
    cache = Cache.put(cache, :a, "a")
    cache = Cache.put(cache, :b, "b")
    cache = Cache.put(cache, :c, "c")
    cache = Cache.put(cache, :d, "d")
    assert %{cap: 0} = cache
    # cache is full, lru (:a) will be evicted
    cache = Cache.put(cache, :e, "e")
    # evicted :a, in cache :b,:c,:d,:e 
    assert {nil, %{cap: 0}} = Cache.get(cache, :a)
    cache = assert_in_cache(cache,[b: "b", c: "c", d: "d", e: "e"])
    # update b value
    cache = Cache.put(cache,:b,"bb")
    # no entries evicted
